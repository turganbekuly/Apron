//
//  SBIResultCollectionCell.swift
//  Apron
//
//  Created by Акарыс Турганбекулы on 25.08.2023.
//
...
protocol SBIResultCollectionCellProtocol: AnyObject {
    func saveRecipeTappedv2(with id: Int)
    func navigateToAuthFromRecipev2()
    func navigateToRecipev2(with id: Int)
}

final class SBIResultCollectionCell: UICollectionViewCell {
    // MARK: - Properties

    weak var delegate: SBIResultCollectionCellProtocol?
    ...
    func configure(with recipe: RecipeResponse) {
        ...
        recipeIngredientsCountLabel.text = "Совпадает \(Int(recipe.sort ?? 0)) из \(recipe.ingredients?.count ?? 0) ингредиентов"
