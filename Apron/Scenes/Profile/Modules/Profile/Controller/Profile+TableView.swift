...
extension ProfileViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .user:
            ...
            return cell
        case .logout, .deleteAccount, .contactWithDevelopers, .myRecipes, .bonus, .favRecipes:
...
extension ProfileViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case .favRecipes:
            let vc = SavedRecipesBuilder(state: .initial(.profile)).build()
            DispatchQueue.main.async {
                self.navigationController?.pushViewController(vc, animated: true)
            }
    ...
    func tableView(_ tableView: UITableView, estimatedHeightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        case .user:
            return 166
        case .bonus, .logout, .assistant, .deleteAccount, .contactWithDevelopers, .myRecipes, .favRecipes:
            return 56
        }
    }
