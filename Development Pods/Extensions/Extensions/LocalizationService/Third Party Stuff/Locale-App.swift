...
public final class AppLocale {
    public fileprivate(set) var original: Locale
    public fileprivate(set) var originalPreferredLanguages: [String]
	private init() {
		original = Locale.current
		originalPreferredLanguages = Locale.preferredLanguages
	}
    public static var shared = AppLocale()
...
extension NSLocale {
	...
	@objc public class var app: Locale {
		return Locale(identifier: AppLocale.identifier)
	}

	@objc public class var appPreferredLanguages: [String] {
...
extension Locale {
	...
	fileprivate static func enforceLanguage(code: String, regionCode: String? = nil) {
        ...
        UserDefaults.zxtDirection = isLanguageRTL
        UserDefaults.forceRTLWrightingDirection = isLanguageRTL
	...
	///   - code: two-letter ISO 639-1 language code
	///   - regionCode: two-letter ISO 3166-1 code
    public static func updateLanguage(code: String, regionCode: String? = nil) {
    ...
    public static func setupInitialLanguage() {
		_ = AppLocale.shared

		NSLocale.swizzle(selector: #selector(getter: NSLocale.current), with: #selector(getter: NSLocale.app))
        let oldSelector = #selector(getter: NSLocale.preferredLanguages)
        let newSelector = #selector(getter: NSLocale.appPreferredLanguages)
		NSLocale.swizzle(selector: oldSelector, with: newSelector)
...
public extension Locale {
    var selectedLanguage: Extensions.Language {
        guard let code = self.languageCode else {
            return .english
        }
        return Extensions.Language(rawValue: code) ?? .russian
    }
}
