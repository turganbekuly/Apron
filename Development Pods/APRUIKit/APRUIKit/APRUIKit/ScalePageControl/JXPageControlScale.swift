...
@IBDesignable open class JXPageControlScale: JXPageControlBase {
        ...
        if leftIndex == rightIndex {
            for index in 0 ..< numberOfPages {
                if index != leftIndex {
                    ...
                    layer.frame = CGRect(
                        x: frame.origin.x - (activeSize.width - inactiveSize.width) * 0.5,
            ...
            leftLayer.backgroundColor = UIColor.transform(
                originColor: tempInactiveColor,
                targetColor: tempActiveColor,
                proportion: leftScare
            ).cgColor
            rightLayer.backgroundColor = UIColor.transform(
                originColor: tempInactiveColor,
                targetColor: tempActiveColor,
                proportion: rightScare
            ).cgColor
            ...
            let leftX = (maxIndicatorSize.width - leftWidth) * 0.5
            + (maxIndicatorSize.width + columnSpacing)
            * CGFloat(leftIndex)

            let rightX = (maxIndicatorSize.width - rightWidth) * 0.5
            + (maxIndicatorSize.width + columnSpacing)
            * CGFloat(rightIndex)

            leftLayer.frame = CGRect(
                x: leftX,
            ...
            for index in 0 ..< numberOfPages where index != leftIndex && index != rightIndex {
                let layer = inactiveLayer[index]
                layer.frame = inactiveOriginFrame[index]
                hollowLayout(layer: layer, isActive: false)
            }
    ...
    override func updateCurrentPage(_ pageIndex: Int) {
        ...
        for index in 0 ..< numberOfPages {
            if index == currentIndex {
                ...
                CATransaction.setAnimationDuration(duration)
                layer.frame = CGRect(
                    x: frame.origin.x - (self.activeSize.width - self.inactiveSize.width) * 0.5,
    ...
    override func layoutInactiveIndicators() {
        inactiveOriginFrame = []
        let xParam = (maxIndicatorSize.width - inactiveSize.width) * 0.5
        let yParam = (maxIndicatorSize.height - inactiveSize.height) * 0.5
        let inactiveWidth = inactiveSize.width > kMinItemWidth ? inactiveSize.width : kMinItemWidth
        let inactiveHeight = inactiveSize.height > kMinItemHeight ? inactiveSize.height : kMinItemHeight
